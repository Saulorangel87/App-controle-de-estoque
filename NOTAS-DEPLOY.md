...
- **Importação de nota fiscal (Fase 3 — OCR de foto/print)**: dois motores
  ...
  - **Print da tela da SEFAZ** → Tesseract local (`services/ocr_nota.go`),
    ...
    seguintes) — **testado em produção: funciona bem**
  - **Foto de cupom de papel físico** → OCR.space na nuvem
    ...
    continua funcionando normal. **Testado em produção pelo celular:
    resultado misto** — uma nota baixada da internet leu 100% certo, mas um
    cupom de papel fotografado só leu 5 de 10 itens, e em outro teste deu
    erro de rede ("Failed to fetch", provavelmente timeout do Cloudflare
    Tunnel na chamada externa à API) — ver "Pendências conhecidas"
...
- **Precisão do OCR.space em foto de cupom de papel real ainda inconsistente**
  ...
  a tela de revisão é quem resolve o resto manualmente
